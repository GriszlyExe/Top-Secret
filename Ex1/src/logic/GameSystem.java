...
public class GameSystem {
	...
	//Fill your code here
		boolean isMotherShip =  (piece instanceof MotherShip) ?true : false;
		boolean isFighter = (piece instanceof Fighter) ? true : false;
		if (isMotherShip) {
			if (attack) {
				Position p = piece.attackTargetPosition(actionPattern);
				removeOtherPieces(piece,p);
			}
			else {
			...
			Position p = piece.attackTargetPosition(actionPattern);
			boolean isRemoved = removeOtherPieces(piece,p);
			if (isRemoved) ((Fighter)piece).setPromoted(true);
		}
